...
This is an implementation of a Elasticnet Regression Forecasting Model in Scikit-Learn as per Ready Tensor specifications. The workflow is as follows:
...
- After this preprocessing of data into tabular format, a Elasticnet Regression model is trained on the tabular data and used to make predictions on the test data.
...
- A **Elasticnet Regression Forecaster** algorithm built using **Scikit-Learn**
...
- Run the script `src/train.py` to train the Elasticnet Regressor classifier model. This will save the model artifacts, including the preprocessing pipeline and label encoder, in the path `./model_inputs_outputs/model/artifacts/`.
